...
In this section, we will evaluate how the PoR protocol performs in a real-world scenario.
...
\begin{figure}
  ...
  \begin{tikzpicture}
    ...
    \pgfplotstableread[col sep=comma]{./data/reputation_loss.csv}\datatable
    \addplot[mark=none, blue] table [x expr=\coordindex, y=reputation] {\datatable};
    \addplot[black, thick] coordinates {(0,0) (100,0)}; % Draws a horizontal line at y=0
    \draw[red, thick, dashed] (axis cs:60,-70) -- (axis cs:60,60) node[pos=1, above] {Peer becomes malicious}; % Draws a vertical line at x=60 with text
    \end{axis}
  \end{tikzpicture}
  \caption{Reputation of a peer, which goes malicious after 60 seconds.}
